...
| 支持连续对象追踪 | 首日按查询点最近对象锚定，后续综合质心距离、对象形态和 bbox 重叠率打分，返回匹配方式、匹配分数、候选数量、状态、位移、速度、方位角和可信度 | `GET /api/front-tracking/{date}`、前端“锋面对象与追踪” |
...
- `GET /api/front-tracking/{date}`：连续日期锋面对象追踪，返回每日状态、匹配方式、匹配分数、候选对象数量、位移、速度、方位角和可信度。
...
- 当前对象追踪已升级为综合质心/形态/bbox 打分 v2，并输出速度、方位角和可信度；但仍是工程演示规则，尚未实现严格的跨日同一锋面轨迹匹配、断裂/合并处理和物理速度约束。
