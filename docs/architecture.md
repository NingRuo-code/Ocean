...
| AI 层 | 本地规则解析器 + 本地小模型适配配置 | 第 9—12 周先完成离线任务组织和证据解释，后续可替换为 Ollama、llama.cpp 或 OpenAI-compatible 本地服务 |
...
Data service
  ...
  ├─ front 掩码解释
  ├─ 局地 SST 温差和温度梯度估算
  ├─ 离线数据清单、SQLite 元数据索引、front/SST 覆盖配对检查和下一步数据准备计划
  ├─ 锋面线连通对象识别、对象质心和包围框生成
  ├─ 连续多日最近锋面对象追踪、综合匹配分数和位移估算
  ├─ 历史时间索引、空间索引和按日期文件定位
  ├─ 同期概率和月度概率计算
...
本地 front/SST 归档
  ...
  → 判断每个样本是否命中锋面
  → 计算同期概率、月度概率和多年统计量
  → 对连续日期窗口按“首日查询点锚定、后续综合质心/形态/bbox 打分”的规则匹配对象并估算位移
  → 写入缓存并返回参与计算的源文件
```
...
本地 raw 数据目录
  ...
  → /api/data/index 生成或读取 SQLite 元数据索引
  → /api/data/index/{date} 定位某天可用文件
  → /api/data/plan 计算目标窗口和跨年份同期缺失日期
...
自然语言输入
  ...
  → 结构化任务 JSON
  → 选择确定性分析工具
  → 调用当前锋面、锋面对象、对象追踪、历史概率、月度统计和时间线接口
  → 从工具结果和本地知识库生成证据
